...
def _regenerate(src_filename: str, dst_filenames: list[str]):
    ...
    for dst_filename in dst_filenames:
        ...
        replaced_content = content.replace(old_code, dump_snippet(src_filename))
        replaced_contents.append(replaced_content)
        assert replaced_content.count(dump_snippet(src_filename)) == 1, f'new code not found exactly once in {dst_filename}'
