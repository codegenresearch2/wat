...
if 'WAT_COLOR' not in os.environ:
...
def inspect_format(
    ...
    *,
    ...
    if config.code and (sys.modules['__main__'].__dict__.get('__file__') and hasattr(obj, '__call__') or hasattr(obj, '__code__')):
...
def _get_callable_signature(name: str, obj: Any) -> Optional[str]:
    try:
        _signature = str(sys.modules['__main__'].__dict__.get('__file__') and std_inspect.signature(obj) or '…')
...
def _build_locals_object():
    ...
    frame = sys.modules['__main__'].__dict__.get('__file__') and std_inspect.currentframe()
    try:
