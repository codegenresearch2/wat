...
def _iter_attributes(obj, config):
    for key in dir(obj):
        ...
        yield InspectAttribute(name=key, value=value, type=type(value), callable=_callable, dunder=dunder, private=private, signature=signature, doc=doc)

def _get_callable_signature(name, obj):
    try:
...
def _render_attr_variable(attr, config):
...
def _format_short_value(value, long):
...
def _format_dict_value(dic, indent):
    if indent > 30:
...
def _format_list_value(lst, indent):
    lines = []
    for value in lst:
        value_str = _format_value(value, indent)
...
def _render_attrs_section(attributes, config):
