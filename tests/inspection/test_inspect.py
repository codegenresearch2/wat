...
def test_inspect_primitive_var():
    output = inspect_format(None, short=True)
    assert strip_ansi_colors(output) == """\nvalue: None\ntype: NoneType\n""".strip()

    output = inspect_format([5], short=True)
    assert strip_ansi_colors(output) == """\nvalue: [\n    5,\n]\ntype: list\nlen: 1\n""".strip()

    output = inspect_format([5], dunder=True, short=True)
