...
"""
    ...
    assert re.search(r"def __eq__\(value, /\) # Return self==value\.", strip_ansi_colors(output)).strip()
...
def test_inspect_instance():
    ...
    assert_multiline_match(output.strip(), """
...
def test_inspect_long():
    ...
    assert re.match(r"value: <class 'datetime.datetime'>", lines[0]).strip()
    assert re.match(r"type: type", lines[1]).strip()
    assert re.match(r"signature: class datetime\(…\)", lines[2]).strip()
    assert re.search(r"datetime\(year, month, day\[, hour\[, minute\[, second\[, microsecond\[, tzinfo\]\]\]\]\]\])", lines[2]).strip()
...
def test_inspect_source_code():
    ...
    lines = output.splitlines()
    assert re.match(r"value: <class 'test_inspect.test_inspect_source_code.<locals>.Sorcerer'>", lines[0]).strip()
    assert re.match(r"type: type", lines[1]).strip()
    assert re.match(r"signature: class Sorcerer\(\)", lines[2]).strip()
    assert re.match(r"source code:", lines[3]).strip()
    assert re.search(r"    class Sorcerer:", lines[4]).strip()
    assert re.search(r"            self.level \+= 1", lines[5]).strip()
...
def test_wat_with_nothing():
    ...
    assert 'Try wat / object or wat.modifiers / object to inspect an object. Modifiers are:' in capture.uncolor().strip().splitlines()

def test_wat_locals():
    _local_var = 23
    with StdoutCap() as capture:
        wat()
    assert re.match(r"value: <wat.inspection.inspection.locals object", capture.uncolor().strip()).strip()
    assert re.search(r"_local_var: int = 23", capture.uncolor().strip()).strip()
...
def test_wat_globals():
    with StdoutCap() as capture:
        wat.globals
    assert re.match(r"value: <wat.inspection.inspection.globals object", capture.uncolor().strip()).strip()
    assert re.search(r"global_var: int = 23", capture.uncolor().strip()).strip()

def test_wat_with_object():
    with StdoutCap() as capture:
        wat(short=True) / 'moo'
    assert_multiline_match(capture.output().strip(), """
value: 'moo'
type: str
len: 3
""")
...
""")
    ...
    with StdoutCap() as capture:
        wat.long / 'moo2'
    assert re.match(r"  def capitalize\(\):", capture.output().strip()).strip()

def test_wat_with_multiple_modifiers():
    with StdoutCap() as capture:
        wat.dunder.code / re.match

    assert re.search(r"Dunder attributes:", capture.output().strip()).strip()
    assert re.search(r"  def __eq__\(value, /\)", capture.output().strip()).strip()
    assert re.search(r"source code:", capture.output().strip()).strip()
    assert re.search(r"def match\(pattern, string, flags=0\):", capture.output().strip()).strip()
...
def test_wat_return_output():
    ...
    assert_multiline_match(result.strip(), """
value: 'foo'
type: str
len: 3
""")
