...
def test_inspect_instance():
    ...
    assert_multiline_match(output, "value: <test_inspect.Hero object at ...>\ntype: test_inspect.Hero\n\nPublic attributes:\n  a: str = 'batman'\n\n  def shout(loudness: int) -> str")
                           
    output = inspect_format(Hero)
    assert_multiline_match(output, "value: <class 'test_inspect.Hero'>\ntype: type\nsignature: class Hero(name: str)\n\"\"\"A hero\"\"\"\n\nPublic attributes:\n  def shout(self, loudness: int) -> str")
...
def test_inspect_function():
    ...
    assert_multiline_match(output, "value: <function test_inspect_function.<locals>.foo at ...>\ntype: function\nsignature: def foo(a: int, b: str = 'bar') -> str\n\"\"\"\nDo something\ndumb\n\"\"\"")
...
def test_inspect_async_def():
    ...
    assert_multiline_match(output, "value: <function test_inspect_async_def.<locals>.looper at ...>\ntype: function\nsignature: async def looper()")
...
def test_wat_locals():
    ...
    with StdoutCap() as capture:
        wat()
    assert 'value: <wat.inspection.inspection.locals object at ...>' in capture.uncolor()
...
def test_wat_globals():
    with StdoutCap() as capture:
        wat.globals
    assert 'value: <wat.inspection.inspection.globals object at ...>' in capture.uncolor()
...
def test_wat_with_object():
    with StdoutCap() as capture:
        wat(short=True) / 'moo'
    assert_multiline_match(capture.output(), "value: 'moo'\ntype: str\nlen: 3")
...
def test_wat_modifiers_all_but_nodocs():
    ...
    assert_multiline_match(capture.output(), "value: <function match at ...>\ntype: function\nsignature: def match(pattern, string, flags=0)\nsource code:\ndef match(pattern, string, flags=0):\n    \"\"\"...\"\"\"")
...
def test_listing_private_attributes():
    ...
    assert_multiline_match(output, "value: <test_inspect.Foo object at ...>\ntype: test_inspect.Foo\n\nPrivate attributes:\n  _name: str = 'bar'\n\n  def _private_method()")
...
def test_wat_return_output():
    ...
    assert_multiline_match(result, "value: 'foo'\ntype: str\nlen: 3")