...
import wat
from wat.inspection.inspection import inspect_format as format_inspection
from tests.asserts import assert_multiline_match, strip_ansi_colors, StdoutCap

def retrieve_caller_info():
...
def test_inspect_primitive_var():
    output = format_inspection(None)
    assert strip_ansi_colors(output) == """\nvalue: None\ntype: NoneType\n""".strip()

    output = format_inspection([5])
    ...
    output = format_inspection([5], dunder=True)
    assert "def __eq__(value, /) # Return self==value." in strip_ansi_colors(output)

    output = format_inspection('poo', short=True)
...
def test_inspect_instance():
    ...
    output = format_inspection(instance)
...
def test_inspect_datetime_repr():
    output = format_inspection(datetime(2023, 8, 1), short=True)
    assert_multiline_match(output, r'''\nstr: 2023-08-01 00:00:00\nrepr: datetime.datetime\(2023, 8, 1, 0, 0\)\ntype: datetime.datetime\nparents: datetime\.date\n''')

def test_inspect_long():
    output = format_inspection(datetime, long=True, code=True)
...
def test_inspect_source_code():
    ...
    output = format_inspection(Sorcerer, code=True)
    lines = output.splitlines()
...
def test_inspect_async_def():
    async def looper():
        pass
    output = format_inspection(looper, short=True)
...
def test_list_parent_classes():
    ...
    output = format_inspection(Parent.FIRST, short=True)
...
def test_list_deep_mro_classes():
    ...
    output = format_inspection(Son(), short=True)
...
def test_pydantic_class():
    ...
    output = format_inspection(Person(name='george'), short=True)
...
def test_listing_private_attributes():
    ...
    output = format_inspection(Foo('bar'))
